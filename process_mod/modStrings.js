...
const civBonusStrings = [
	...
	"Blacksmith attack upgrades affect bonus damage",
...
var uniqueCastleStrings = [
	...
	"Medical Corps (Elephants units regenerate 30 HP per minute)",
	...
	"Replaceable Parts (Siege +1/+1P armor, repairing siege is free)",
];
...
var uniqueImpStrings = [
	...
	"Atheism (+100 years for Relic, Wonder victories; enemy relics generate -50% resources)",
	"this text will change",
	...
	"Lamellar Armor (Camels and Cavalry Archers +2/+1P armor)",
	...
	"Gate Crashing (Ram gold cost is replaced by additional wood cost)",
...
function interperateLanguage(json_path, txt_path) {
	...
	stringStream.write('8252 "Research Royal Heirs (' + getUniqueName(10, "castletech") + ' and Camels receive -3 damage from Mounted Units)"\n');
	stringStream.write(
		'28252 "Research <b>Royal Heirs<b> (<cost>) \\n' + getUniqueName(10, "castletech") + ' and Camels receive -3 damage from Mounted Units."\n'
	);
	...
	stringStream.write(
		'8344 "Research First Crusade (Each Town Center (maximum 5) spawns a one-time batch of 7 ' +
			getUniqueName(29, "castletech") +
			'; units more resistant to conversion)"\n'
	);
	stringStream.write(
		'28344 "Research <b>First Crusade<b> (<cost>) \\nEach Town Center (maximum 5) spawns a one-time batch of 7 ' +
			getUniqueName(29, "castletech") +
			'; units more resistant to conversion."\n'
	);
	...
	uniqueCastleStrings[29] =
		"First Crusade (Each Town Center (maximum 5) spawns a one-time batch of 7 " + getUniqueName(29, "castletech") + "; units more resistant to conversion)";
	...
	for (var i = 0; i < civs.techtree.length; i++) {
		...
		for (var j = 0; j < civs.civ_bonus[i].length; j++) {
			if (!Array.isArray(civs.civ_bonus[i][j])) {
				stringStream.write("• " + civBonusStrings[civs.civ_bonus[i][j]] + " \\n");
			} else {
				if (civs.civ_bonus[i][j][1] > 1) {
					stringStream.write("• " + civBonusStrings[civs.civ_bonus[i][j][0]] + " [x" + civs.civ_bonus[i][j][1] + "]\\n");
				} else {
					stringStream.write("• " + civBonusStrings[civs.civ_bonus[i][j][0]] + " \\n");
				}
			}
		}
		...
		for (var j = 0; j < civs.castletech[i].length; j++) {
			if (!Array.isArray(civs.castletech[i][j])) {
				stringStream.write("• " + uniqueCastleStrings[civs.castletech[i][j]] + " \\n");
			} else {
				if (civs.castletech[i][j][1] > 1) {
					stringStream.write("• " + uniqueCastleStrings[civs.castletech[i][j][0]] + " [x" + civs.castletech[i][j][1] + "]\\n");
				} else {
					stringStream.write("• " + uniqueCastleStrings[civs.castletech[i][j][0]] + " \\n");
				}
			}
		}
		for (var j = 0; j < civs.imptech[i].length; j++) {
			if (!Array.isArray(civs.imptech[i][j])) {
				stringStream.write("• " + uniqueImpStrings[civs.imptech[i][j]] + " \\n");
			} else {
				if (civs.imptech[i][j][1] > 1) {
					stringStream.write("• " + uniqueImpStrings[civs.imptech[i][j][0]] + " [x" + civs.imptech[i][j][1] + "]\\n");
				} else {
					stringStream.write("• " + uniqueImpStrings[civs.imptech[i][j][0]] + " \\n");
				}
			}
		}
		stringStream.write("\\n");
		...
		for (var j = 0; j < civs.team_bonus[i].length; j++) {
			if (!Array.isArray(civs.team_bonus[i][j])) {
				stringStream.write("• " + teamBonusStrings[civs.team_bonus[i][j]] + "\\n");
			} else {
				if (civs.team_bonus[i][j][1] > 1) {
					stringStream.write("• " + teamBonusStrings[civs.team_bonus[i][j][0]] + " [x" + civs.team_bonus[i][j][1] + "]\\n");
				} else {
					stringStream.write("• " + teamBonusStrings[civs.team_bonus[i][j][0]] + "\\n");
				}
			}
		}
		stringStream.write('"\n');
	...
	stringStream.write('7510 "Replaceable Parts"\n');
	stringStream.write('8510 "Research Replaceable Parts (Siege units +1/+1P armor, repairing siege is free)"\n');
	stringStream.write('28510 "Research <b>Replaceable Parts<b> (<cost>)\\nSiege units +1/+1P armor, repairing siege is free."\n');
	stringStream.write('17510 "Replaceable\\nParts"\n');
	stringStream.write('7511 "Pila"\n');
	stringStream.write('8511 "Pila (Skirmisher attacks strip armor)"\n');
	stringStream.write('28511 "Research <b>Pila<b> (<cost>)\\nSkirmisher attacks strip armor."\n');
	stringStream.write('17511 "Pila"\n');
	stringStream.write('7512 "Enlistment"\n');
	stringStream.write('8512 "Research Enlistment (Infantry take 15% less population space)"\n');
	stringStream.write('28512 "Research <b>Enlistment<b> (<cost>)\\nInfantry take 15% less population space."\n');
	stringStream.write('17512 "Enlistment"\n');
	stringStream.write('7513 "Marshalled Hunters"\n');
	stringStream.write('8513 "Research Marshalled Hunters (Foot archers and skirmishers take 15% less population space)"\n');
	stringStream.write('28513 "Research <b>Marshalled Hunters<b> (<cost>)\\nFoot archers and skirmishers take 15% less population space."\n');
	stringStream.write('17513 "Marshalled\\nHunters"\n');
	stringStream.write('7514 "Shigeto Yumi"\n');
	stringStream.write(
