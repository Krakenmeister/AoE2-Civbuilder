...
function startaBuild() {
  ...
  combineButton.onclick = function () {
    ...
    vanillaText.innerHTML =
      "&emsp;1) &nbsp;Select civilization .json files to combine into a mod.zip file. Note that all other civilizations' bonuses will be overwritten in this mod.<br>" +
      ...
      function onChange(event) {
        ...
        Promise.all(readers).then(async (values) => {
          if (checkCompatibility(values) === 0) {
            ...
            //   a.click();
            // });
            post(`${route}/create`, { presets: raw_presets, seed: dataSeed });
          }
        });
      }
