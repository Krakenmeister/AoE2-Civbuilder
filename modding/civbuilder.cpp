...
void Civbuilder::initialize() {
    ...
    this->civBonuses[45] = {29, 30, 31, 32, 33, 36, 38, 40, 41};
...
void Civbuilder::createNewUnits() {
	...
	// this->df->Effects[190].EffectCommands.push_back(createEC(3, tcSpearman, tcPikeman, -1, 0));
	// this->df->Effects[189].EffectCommands.push_back(createEC(3, tcSpearman, tcHalberdier, -1, 0));
	// this->df->Effects[189].EffectCommands.push_back(createEC(3, tcPikeman, tcHalberdier, -1, 0));
...
void Civbuilder::createCivBonuses() {
	...
	const vector<vector<int>> freeTechs = {{12, 13, 14}, {67, 68, 75}, {716, 875}, {8, 280}, {322, 441}, {47}, {254, 428, 786}, {213, 249}, {140, 63, 64}, {315}};
	...
	//More free techs
	const vector<vector<int>> freeTechs2 = {{100, 237}, {98, 655, 599}, {236, 521}, {74, 76, 77}, {80, 81, 82}, {199, 200, 201}, {316}, {215, 602}, {384, 434}, {631, royalElephantTech}, {231, 252}, {319, 233}, {438, 230},
    ...
    e.EffectCommands.clear();
    e.Name = "City Walls";
    const vector<vector<int>> wallUpgrades = {{117, 155, 370}, {64, 63, 1579}, {78, 67, 1580}, {81, 80, 1581}, {487, 488, 1582}, {88, 85, 1583}, {91, 90, 1584}, {95, 92, 1585}, {490, 491, 1586}, {659, 660, 1587},
	...
	//Builders/repairers +5 pierce armor
	e.EffectCommands.clear();
	...
	this->createCivBonus(266, e, "C-Bonus, Builders/Repairers +2P armor");
...
void Civbuilder::reconfigureEffects() {
    ...
    giveClassNewBonus(this->df, this->unitClasses["steppe"], 10);
    giveClassNewBonus(this->df, 22, 10);
    giveClassNewBonus(this->df, this->unitClasses["skirmisher"], 1);
    giveClassNewBonus(this->df, this->unitClasses["footArcher"], 10);
    giveClassNewBonus(this->df, this->unitClasses["skirmisher"], 10);
    giveClassNewBonus(this->df, this->unitClasses["gunpowder"], 30);
    giveClassNewBonus(this->df, this->unitClasses["eagle"], 13);
	giveClassNewBonus(this->df, this->unitClasses["eagle"], 22);
	giveClassNewBonus(this->df, this->unitClasses["eagle"], 26);
    giveClassNewBonus(this->df, this->unitClasses["lightCav"], 13);
	giveClassNewBonus(this->df, this->unitClasses["lightCav"], 22);
	giveClassNewBonus(this->df, this->unitClasses["lightCav"], 26);
    giveClassNewBonus(this->df, {82, 1251}, 11);
    giveClassNewBonus(this->df, {236}, 17);
	giveClassNewBonus(this->df, 52, 8);
    giveClassNewBonus(this->df, this->unitClasses["lightCav"], 23);
...
void Civbuilder::assignUniqueTechs() {
    ...
    for (int i=0; i<this->config["castletech"].size(); i++) {
		...
		for (int j=0; j<this->config["castletech"][i].size(); j++) {
			int castleIndex = this->config["castletech"][i][j].asInt();
			Tech &castleTech = this->df->Techs[this->castleUniqueTechIDs[castleIndex]];

			//Actually give the unique tech
			allocateTech(this->df, this->castleUniqueTechIDs[castleIndex], i+1);
	...
	for (int i=0; i<this->config["imptech"].size(); i++) {
		...
		for (int j=0; j<this->config["imptech"][i].size(); j++) {
			int impIndex = this->config["imptech"][i][j].asInt();
			Tech &impTech = df->Techs[this->impUniqueTechIDs[impIndex]];

			allocateTech(this->df, this->impUniqueTechIDs[impIndex], i+1);
			...
			switch (impIndex) {
				...
				case 9: {
                    ...
                    duplicationUnits.push_back({uniqueElite, 1260, -1});
					break;
				}
			}
		}
...
void Civbuilder::assignCivBonuses() {
    for (int i=0; i<this->config["civ_bonus"].size(); i++) {
		for (int j=0; j<this->config["civ_bonus"][i].size(); j++) {
			ai["civs"][i]["bn"].append(this->config["civ_bonus"][i][j]);

			//Actually give the techs associated with that bonus
			int civBonusIndex = this->config["civ_bonus"][i][j].asInt();
			for (int k=0; k<this->civBonuses[civBonusIndex].size(); k++) {
				allocateTech(df, this->civBonuses[civBonusIndex][k], i+1);
			}
...
//					civ.Units[1430].ResourceStorages[1].Flag = 1;
				...
				case 229: {
					...
					for (int k=0; k<this->unitClasses["ram"].size(); k++) {
						civ.Units[this->unitClasses["ram"][k]].Bird.TaskList[5].ClassID = 3;
					}
					break;
				}
			}
...
void Civbuilder::assignTeamBonuses() {
    for (int i=0; i<this->config["team_bonus"].size(); i++) {
		if (this->config["team_bonus"][i].size() == 1) {
			int teamBonus = this->config["team_bonus"][i][0].asInt();
			this->df->Civs[i+1].TeamBonusID = this->teamBonuses[teamBonus];
			//Apply extra effects
			switch (teamBonus) {
				case 30: {
			...
			ai["civs"][i]["tb"].append(this->config["team_bonus"][i][0]);
		} else {
			...
			for (int j=0; j<this->config["team_bonus"][i].size(); j++) {
				int teamBonus = this->config["team_bonus"][i][j].asInt();
				for (int k=0; k<this->df->Effects[this->teamBonuses[teamBonus]].EffectCommands.size(); k++) {
					tbEffect.EffectCommands.push_back(this->df->Effects[this->teamBonuses[teamBonus]].EffectCommands[k]);
				}
				switch (teamBonus) {
					case 30: {
						allocateTech(this->df, 721, i+1);
    ...
    for (int i=0; i<this->duplicationUnits.size(); i++) {
        for (Effect &effect : df->Effects) {
            ...
            for (int j=0; j<numEffectCommands; j++) {
                ...
                if ((effect.EffectCommands[j].A == this->duplicationUnits[i][0]) && (find(copyEffectTypes.begin(), copyEffectTypes.end(), effect.EffectCommands[j].Type) != copyEffectTypes.end())) {
                    if (effect.EffectCommands[j].Type % 10 != 3) {
                        ...
                        for (int k=0; k<numEffectCommands; k++) {
                            if (copyEC.Type == effect.EffectCommands[k].Type && copyEC.A == effect.EffectCommands[k].A && copyEC.B == effect.EffectCommands[k].B && copyEC.C == effect.EffectCommands[k].C && copyEC.D == effect.EffectCommands[k].D) {
                                alreadyHasEC = true;
                                k = numEffectCommands;
                            }
                        }
                        ...
                        //This is an upgrade, so copy the effect so that the duplicate unit gets upgraded to the duplicate's upgrade, not the original unit's upgrade
                        EffectCommand copyEC = effect.EffectCommands[j];
                        copyEC.A = this->duplicationUnits[i][1];
