...
const card_descriptions = [
  ...
  [
    ...
    "Chieftans (Infantry deal bonus damage to cavalry) [600F 450G]",
...
function decryptPath(path) {
  ...
  civ.tree = [[], [], []];
  const encryptedTree = asciiToBinary(path.substring(aliasLength + separatorLength + flagPaletteLength, aliasLength + separatorLength + flagPaletteLength + techtreeLength));
  for (let i = 0; i < treeMap.length; i++) {
  ...
  civ.architecture = parseInt(
    asciiToBinary(path.substring(aliasLength + separatorLength + flagPaletteLength + techtreeLength, aliasLength + separatorLength + flagPaletteLength + techtreeLength + architectureLength)),
    2
  );
