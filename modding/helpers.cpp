...
float
amountTypetoD(int32_t value, int32_t type) {
	int32_t NewD = 0;
...
EffectCommand
createEC(int type, int A, int B, int C, float D) {
		...
		if (effect.Name == "Elite costs -20%") {
						...
						for (int j = 0; j < 3; j++) {
							if (j < df->Techs[i].ResourceCosts.size()) {
								if ((df->Techs[i].ResourceCosts[j].Type >= 0) && (df->Techs[i].ResourceCosts[j].Type <= 3)) {
									effect.EffectCommands.push_back(
										createEC(101, i, df->Techs[i].ResourceCosts[j].Type, -1, -1 * (df->Techs[i].ResourceCosts[j].Amount / 5)));
								}
							}
						}
					}
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					if ((df->Techs[i + 101].ResourceCosts[j].Type >= 0) && (df->Techs[i + 101].ResourceCosts[j].Type <= 3)) {
						effect.EffectCommands.push_back(createEC(101, i + 101, df->Techs[i + 101].ResourceCosts[j].Type, -1,
																 (int) round(-1 * (df->Techs[i + 101].ResourceCosts[j].Amount * 0.15))));
					}
				}
			}
			...
			for (int i = 0; i < 3; i++) {
				if ((df->Techs[103].ResourceCosts[i].Type >= 0) && (df->Techs[103].ResourceCosts[i].Type <= 3)) {
					effect.EffectCommands.push_back(
						createEC(101, 103, df->Techs[103].ResourceCosts[i].Type, -1, -1 * (df->Techs[103].ResourceCosts[i].Amount / 3)));
				}
			}
			...
			for (int i = 0; i < df->Techs.size(); i++) {
				if ((df->Techs[i].ResearchLocation == 45) || (df->Techs[i].ResearchLocation == 209)) {
					for (int j = 0; j < 3; j++) {
						if ((df->Techs[i].ResourceCosts[j].Type >= 0) && (df->Techs[i].ResourceCosts[j].Type <= 3)) {
							effect.EffectCommands.push_back(
								createEC(101, i, df->Techs[i].ResourceCosts[j].Type, -1, -1 * (df->Techs[i].ResourceCosts[j].Amount / 3)));
						}
					}
				}
			}
			...
			for (int i = 0; i < df->Techs.size(); i++) {
				if (df->Techs[i].ResearchLocation == 101) {
					for (int j = 0; j < 3; j++) {
						if ((df->Techs[i].ResourceCosts[j].Type >= 0) && (df->Techs[i].ResourceCosts[j].Type <= 3)) {
							effect.EffectCommands.push_back(
								createEC(101, i, df->Techs[i].ResourceCosts[j].Type, -1, -1 * (df->Techs[i].ResourceCosts[j].Amount / 2)));
						}
					}
				}
			}
			...
			effect.EffectCommands.clear();
			for (int i = 0; i < df->Techs.size(); i++) {
				if ((df->Techs[i].ResearchLocation == 562) || (df->Techs[i].ResearchLocation == 68) || (df->Techs[i].ResearchLocation == 584) ||
					(df->Techs[i].ResearchLocation == 84) || (i == 65)) {
					for (int j = 0; j < 3; j++) {
						if (df->Techs[i].ResourceCosts[j].Type == 0) {
			...
			for (int i = 0; i < 3; i++) {
				if ((df->Techs[101].ResourceCosts[i].Type >= 0) && (df->Techs[101].ResourceCosts[i].Type <= 3)) {
					effect.EffectCommands.push_back(
						createEC(101, 101, df->Techs[101].ResourceCosts[i].Type, -1, -1 * (df->Techs[101].ResourceCosts[i].Amount / 4)));
				}
			}
			...
			for (int i = 0; i < 3; i++) {
				if ((df->Techs[102].ResourceCosts[i].Type >= 0) && (df->Techs[102].ResourceCosts[i].Type <= 3)) {
					effect.EffectCommands.push_back(
						createEC(101, 102, df->Techs[102].ResourceCosts[i].Type, -1, -1 * (df->Techs[102].ResourceCosts[i].Amount / 4)));
				}
			}
...
void
copyArchitecture(DatFile *df, int copyFrom, int copyTo) {
	for (int i = 0; i < df->Civs[copyFrom].Units.size(); i++) {
		if ((df->Civs[copyFrom].Units[i].Class == 3) || (df->Civs[copyFrom].Units[i].Class == 52) || (df->Civs[copyFrom].Units[i].Class == 27) ||
			(df->Civs[copyFrom].Units[i].Class == 39)) {
		...
		} else if ((df->Civs[copyFrom].Units[i].Class == 59) || (df->Civs[copyFrom].Units[i].Class == 18) || (df->Civs[copyFrom].Units[i].Class == 43) ||
				   (df->Civs[copyFrom].Units[i].Class == 19) || (df->Civs[copyFrom].Units[i].Class == 22)) {
			df->Civs[copyTo].Units[i].StandingGraphic = df->Civs[copyFrom].Units[i].StandingGraphic;
			df->Civs[copyTo].Units[i].DyingGraphic = df->Civs[copyFrom].Units[i].DyingGraphic;
			df->Civs[copyTo].Units[i].UndeadGraphic = df->Civs[copyFrom].Units[i].UndeadGraphic;
...
void
giveClassNewBonus(DatFile *df, int classID, int bonusID) {
	for (Civ &civ : df->Civs) {
...
void
giveUnitsNewBonus(DatFile *df, vector<int> unitIDs, int bonusID) {
	for (Civ &civ : df->Civs) {
...
void
giveClassNewArmor(DatFile *df, vector<int> unitIDs, int armorID, int armorAmount) {
	for (Civ &civ : df->Civs) {
		for (int i = 0; i < unitIDs.size(); i++) {
			int hasArmor = -1;
...
void
setCombatStats(DatFile *df, int unitID, vector<vector<int>> attacks, vector<vector<int>> armors) {
	for (Civ &civ : df->Civs) {
...
void
addAttacktoUnits(DatFile *df, int effectID, int attackAmount, vector<int> units) {
...
void
setUnitCosts(DatFile *df, vector<int> costs, int civID, int unitID) {
...
void
setUnitCosts(DatFile *df, vector<int> unitIDs, vector<int> costs) {
...
void
allocateRequirements(DatFile *df, int techID, int civID) {
...
multiplyEffects(DatFile *df, int effectID, int multiplier) {
	...
	for (int i = 0; i < initialEffectSize; i++) {
		for (int j = 0; j < multiplier - 1; j++) {
			EffectCommand multiplyEC = df->Effects[effectID].EffectCommands[i];
			df->Effects[effectID].EffectCommands.push_back(multiplyEC);
		}
	}
}
...
multiplyAllEffects(DatFile *df, vector<vector<int>> multipliedTechs) {
	...
	for (int i = 0; i < multiplieds.size(); i++) {
		// Check for techs that use the same effect
		bool hasDifferentAhead = false;
		int hasSameBehind = -1;
		for (int j = 0; j < multiplieds.size(); j++) {
			if (multiplieds[i][1] == multiplieds[j][1]) {
				if (j < i && multiplieds[i][2] == multiplieds[j][2]) {
					hasSameBehind = j;
				} else if (j > i && multiplieds[i][2] != multiplieds[j][2]) {
					hasDifferentAhead = true;
				}
			}
		}

		if (hasSameBehind != -1) {
			df->Techs[multiplieds[i][0]].EffectID = df->Techs[multiplieds[hasSameBehind][0]].EffectID;
		} else if (hasDifferentAhead) {
			// If we have a tech in the future that will require a different multiplier amount, duplicate this one and then multiply it
			Effect duplicateEffect = Effect();
			duplicateEffect.Name = df->Effects[multiplieds[i][1]].Name;
			duplicateEffect.EffectCommands = df->Effects[multiplieds[i][1]].EffectCommands;
			df->Effects.push_back(duplicateEffect);
			df->Techs[multiplieds[i][0]].EffectID = (int) (df->Effects.size() - 1);
			multiplyEffects(df, df->Techs[multiplieds[i][0]].EffectID, multiplieds[i][2]);
		} else {
			// Just multiply this effect, no one else needs it
			multiplyEffects(df, multiplieds[i][1], multiplieds[i][2]);
		}
	}
}
...
int
allocateTech(DatFile *df, int techID, int civID /*, ofstream &logfile*/) {
	...
	if (df->Techs[techID].Civ == 99) {
		...
		// This will only exist if you previously allocated a tech that created a duplicate of the same tech you are trying to allocate now (because it needed
		// to fulfill its required techs)
		bool existingDuplicate = false;
		int newTechID = -1;
		for (int i = 0; i < df->Techs.size(); i++) {
				...
				if ((tech.Civ == -1) || (tech.Civ == civID)) {
					for (int i = 0; i < tech.RequiredTechs.size(); i++) {
						if (tech.RequiredTechs[i] == techID) {
							// For some reason all techs in base-game start with size 6 which makes the hasRequired variable necessary (and numRequiredTechs
							// variable later) Fortunately, the Tech default constructor gives it size 0 which is why this is only necessary here
...
vector<vector<int>> unitSets = {
	...
	/*13 - Markets*/ {300, 3000, 5, 3, 84, 116, 137},
	...
	/*23 - Stone Gates*/ {25,	50,	  7,	3,	  63,	64,	  67,	78,	  80,	81,	  85,	88,	  90,	91,	  92,	95,	 487, 488,
						  490,	491,  659,	660,  661,	662,  663,	664,  665,	666,  667,	668,  669,	670,  671,	672, 673, 674,
	...
	/*38 - Knights*/ {85, 185, 0, 2, 38, 283, 569, 1813},
	...
	/*72 - Mule Cart*/ {50, 150, 0, 3, 1808},
	/*73 - Fortified Church*/ {100, 300, 7, 3, 1806},
	/*74 - Warrior Priest*/ {50, 120, 0, 2, 1811, 1831},
};
vector<int> uniqueUnits = {8,	 530,  281,	 531,  41,	 555,  25,	 554,  291,	 560,  73,	 559,  40,	 553,  282,	 556,  239,	 558,  46,
						   ...
						   1701, 1702, 1704, 1706, 1735, 1737, 1738, 1740, 1741, 1743, 1759, 1761, 1800, 1802, 1803, 1805};
...
vector<int>
getRandomCosts(int min, int max, int p, int maxTypes) {
...
void
setTechCosts(DatFile *df, vector<int> costs, int techID) {
...
// Apply modifiers
void
applyModifiers(DatFile *df, bool blindnessModifier, double buildingMultiplier, double speedMultiplier, double healthMultiplier) {
	// Apply blind modifier
	if (blindnessModifier) {
		// Give all units maximum 1 LOS
		for (Civ &civ : df->Civs) {
			for (Unit &unit : civ.Units) {
		...
		// Remove all effects that increase LOS
		for (Effect &effect : df->Effects) {
	...
	// Apply building work rate modifier
	for (Civ &civ : df->Civs) {
		for (Unit &unit : civ.Units) {
			if (unit.Class == 3) {
				unit.Bird.WorkRate *= buildingMultiplier;
			}
		}
	}
	...
	if (healthMultiplier != 0) {
		for (Civ &civ : df->Civs) {
			for (Unit &unit : civ.Units) {
				unit.HitPoints *= healthMultiplier;
			}
		}
	} else {
		// Set all hp to 1
		...
		// Remove all effects that modify hit points
		for (Effect &effect : df->Effects) {
			for (EffectCommand &ec : effect.EffectCommands) {
				if ((ec.Type == 4 || ec.Type == 5) && ec.C == 0) {
					bool appliesToBuilding = false;
					if (ec.A != -1) {
						// If it effectcommand to a unit, check if that unit's class is a building class
						for (int i = 0; i < buildingClasses.size(); i++) {
							if (df->Civs[0].Units[ec.A].Class == buildingClasses[i]) {
								appliesToBuilding = true;
							}
						}
					} else if (ec.B != -1) {
						// If the effectcommand applies to a class, check if it is a building class
						for (int i = 0; i < buildingClasses.size(); i++) {
							if (ec.B == buildingClasses[i]) {
								appliesToBuilding = true;
							}
						}
					}

					if (!appliesToBuilding) {
						if (ec.Type == 4) {
							ec.D = 0;
						} else if (ec.Type == 5) {
							ec.D = 1;
