const card_descriptions = [
	...
	"Elephant Archers (ranged elephants)",
	...
	"Legionaries (heavily armoured infantry)",
	...
	"Howdah (Elephant units +1/+2P armor) [400F 300W]",
	"Greek Fire (Fire ships +1 range) [250F 300G]",
	"Stronghold (Castles and towers fire 25% faster) [250F 200G]",
	...
	"Royal Heirs (Unique Unit trains faster) [300F 300G]",
	...
	"Andean Sling (Skirmishers and Slingers no minimum range) [200F 300G]",
	"Sultans (All gold income 10% faster) [400F 400W]",
	...
	"Madrasah (Monks return 33% of their cost when killed) [200F 100G]",
	"First Crusade (Each Town Center (maxiumum 5) spawns a one-time batch of 7 of your Unique Unit; units are more resistant to conversion) [300F 600G]",
	"Orthodoxy (Monk units +3/+3P armor) [200F 300G]",
	"Inquisition (Monks convert faster) [100F 300G]",
	...
	"Flemish Revolution (Upgrades all existing Villagers to Flemish Militia; create Flemish Militia at Town Centers) [800F 450G]",
	"Manipur Cavalry (Cavalry +5 attack vs. archers) [650F 400G]",
	...
	"Elite Mercenaries (team members can train 10 free elite versions of your Unique Unit in the castle) [650F 400G]",
	...
	"Atheism (+100 years for Relic, Wonder victories; enemy relics generate -50% resources) [500F 500G]",
	"Fabric Shields (Eagles, Slingers, Unique Unit +1/+2 armor) [600F 600G]",
	"Shatagni (Hand Cannoneers +1 range) [500F 300G]",
	"Silk Road (Trade units cost -50%) [500F 250G]",
	"Kataparuto (Trebuchets fire, pack faster) [750W 400G]",
	...
	"Zealotry (Camel units +20 hit points) [500F 450G]",
	"Hauberk (Knights +1/+2P armor) [500F 400G]",
	"Druzhina (Infantry damage adjacent units) [1200F 500G]",
	...
	"Artillery (+2 range Bombard Towers, Bombard Cannons, Cannon Galleons) [500G 450S]",
	...
	"Outposts constructed nearly instantly",
	"Siege towers don't cost gold",
...
//	console.log(String.fromCharCode(92))
	...
	civ.tree = [[], [], []];
	const encryptedTree = asciiToBinary(path.substring(aliasLength+1+6, aliasLength+1+6+28));
	for (let i=0; i<treeMap.length; i++) {
	...
	while (true) {
		if (path.substring(left, right) != '') {
			civ.bonuses[cardType].push(parseInt(path.substring(left, right)));
		}
