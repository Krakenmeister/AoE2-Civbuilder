...
function displayData(relativepath = ".") {
  ...
  let age_names = [data.strings[data.age_names["Dark Age"]], data.strings[data.age_names["Feudal Age"]], data.strings[data.age_names["Castle Age"]], data.strings[data.age_names["Imperial Age"]]];
  ...
  for (let lane of tree.lanes) {
    draw
      .rect(lane.width + 10, tree.height)
...
function getHelpText(name, id, type) {
  ...
  text = text.replace(/\n/g, "");
  if (type === "TECHNOLOGY") {
    text = text.replace(/(.+?\(.+?\))(.*)/m, '<p class="helptext__heading">$1</p>' + '<p class="helptext__desc">$2</p>' + '<p class="helptext__stats">&nbsp;</p>');
  } else if (type === "UNIT" || type === "UNIQUEUNIT") {
    text = text.replace(/(.+?\(‹cost›\))(.+?)<i>\s*(.+?)<\/i>(.*)/m, "<p>$1</p>" + "<p>$2</p>" + "<p><em>$3</em></p>" + '<p class="helptext__stats">$4</p>');
