const fs = require("fs");
const { iconids, techtreeNames, nameArr } = require("./constants.js");
...
const computer_name_string_table_offsets = [
	4400, 4420, 4440, 4460, 4480, 4500, 4520, 4540, 4560, 4580, 4600, 4620, 4640, 5800, 5820, 5840, 5860, 5880, 5900, 5920, 5940, 5960, 5980, 106000, 106020,
	106040, 106060, 106080, 106100, 106120, 106140, 106160, 106180, 106200, 106220, 106240, 106260, 106280, 106300, 106320, 106340, 106360, 106380, 106400,
	106420,
];
...
function createCivilizationsJson(data_json, civilizations_json) {
	...
	for (let i = 0; i < civ.techtree.length; i++) {
		...
		civilization.tech_tree_image_path = `/resources/civ_techtree/menu_techtree_${nameArr[i]}.png`;

		civilization.unique_unit_image_paths = [];
		civilization.unique_unit_image_paths.push(`/resources/uniticons/${iconids[civ.techtree[i][0]]}_50730.png`);
