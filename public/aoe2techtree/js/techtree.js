...
function printAllCarets(tree) {
	let string = "[";
	let bitpos = 0;
	for (let lane of tree.lanes) {
		for (let r of Object.keys(lane.rows)) {
			for (let caret of lane.rows[r]) {
				string = string + "[" + qidType(caret.id) + ", " + qidID(caret.id) + "],\n";
				bitpos++;
			}
		}
	}
...
function unique(ids, monk_prefix) {
	...
	SVG.get("unit_" + formatId(UNIQUE_UNIT) + "_img").load("img/Units/" + formatId(ids[0]) + ".jpg");
	SVG.get("unit_" + formatId(ELITE_UNIQUE_UNIT) + "_img").load("img/Units/" + formatId(ids[1]) + ".jpg");
	SVG.get("unit_" + formatId(MONK) + "_img").load("img/Units/" + monk_prefix + "125.jpg");
}
