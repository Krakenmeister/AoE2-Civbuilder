...
function startaBuild() {
	...
	editButton.onclick = function () {
		...
		(function () {
			...
			function onReaderLoad(event) {
				var civ = JSON.parse(event.target.result);
				location.href = "/edit/" + encryptJson(civ);
			}
	...
	combineButton.onclick = function () {
		...
		vanillaText.innerHTML = "&emsp;1) &nbsp;Select civilization .json files to combine into a mod.zip file. Note that all other civilizations' bonuses will be overwritten in this mod.<br>" + "&emsp;2) &nbsp;To include vanilla civilizations in your mods, click to download the .json files for all the civilizations in the base game.";
		...
		(function () {
			function checkCompatibility(presets) {
				...
				var errorMessage;
				const errorMessages = ["Duplicate Krepost civs!", "Duplicate Donjon civs!", "Duplicate Anarchy civs!", "Duplicate Marauder civs!", "Duplicate First Crusade civs!", "Duplicate Elite Mercenaries civs!", "Duplicate relic bonus civs!", "Duplicate Royal Heirs civs!", "Duplicate Bearded Axe civs!", "Duplicate Pavise civs!", "Duplicate Corvinian Army civs!", "Duplicate Manipur Cavalry civs!", "Duplicate Logistica civs!", "Duplicate Rocketry civs!", "Duplicate Fabric Shields civs!", "Duplicate Berserkergang civs!"];
