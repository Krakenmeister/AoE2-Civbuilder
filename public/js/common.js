...
// Civilization bonuses, unique units, castle techs, imp techs, team bonuses
...
function validate(value, type) {
	...
	if (value.length > 30) {
		alert("Please enter a shorter " + type);
		return false;
	}
	return true;
}
...
function encryptJson(civ) {
	...
	path += civ.description.replaceAll(" ", ":");
	path += "!";
	...
	if (civ.wonder) {
		binaryString = zeroFill(civ.wonder.toString(2), 6);
		path += binaryToAscii(binaryString);
	} else {
		path += binaryToAscii("000000");
	}
	...
	if (civ.castle) {
		binaryString = zeroFill(civ.castle.toString(2), 6);
		path += binaryToAscii(binaryString);
	} else {
		path += binaryToAscii("000000");
	}
...
function decryptPath(path) {
	...
	const wonderLength = 1;
	const castleLength = 1;
	...
	readIndex += aliasLength;

	const descriptionLength = path.indexOf("!", readIndex + 1) - readIndex;
	civ.description = path.substring(readIndex + 1, readIndex + descriptionLength).replaceAll(":", " ");

	readIndex += descriptionLength;

	console.log(civ.description);

	civ.flag_palette = [];
	const encryptedPalette = asciiToBinary(path.substring(readIndex + 1, readIndex + 1 + 6));
	for (let i = 0; i < 8; i++) {
		if (i == 0) {
	...
	readIndex += separatorLength + flagPaletteLength;

	civ.tree = [[], [], []];
	const encryptedTree = asciiToBinary(path.substring(readIndex, readIndex + techtreeLength));
	for (let i = 0; i < treeMap.length; i++) {
		if (encryptedTree[i] == "1") {
			civ.tree[treeMap[i][0]].push(treeMap[i][1]);
		}
	}
	readIndex += techtreeLength;

	civ.architecture = parseInt(asciiToBinary(path.substring(readIndex, readIndex + architectureLength)), 2);
	readIndex += architectureLength;

	civ.language = parseInt(asciiToBinary(path.substring(readIndex, readIndex + languageLength)), 2);
	readIndex += languageLength;

	civ.wonder = parseInt(asciiToBinary(path.substring(readIndex, readIndex + wonderLength)), 2);
	readIndex += wonderLength;

	civ.castle = parseInt(asciiToBinary(path.substring(readIndex, readIndex + castleLength)), 2);
	readIndex += castleLength;

	civ.bonuses = [[], [], [], [], []];
	let cardType = 0;
	path += ":;";
	let left = readIndex + 1;
...
function filterCards(filterText) {
	for (let i = 0; i < num_cards[roundType]; i++) {
		...
		let edition = card_descriptions[roundType][i][2];
		if (edition < 0) {
			edition = 0;
		}
		if (filterRarities[card_descriptions[roundType][i][1]] == false || filterEditions[edition] == false) {
			cardVisible = false;
		}
		if (filterText != "") {
